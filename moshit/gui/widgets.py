...
class MediaLibrary(QWidget):
    importRequested = Signal(str)             # role: "main" | "motion"
    addRequested = Signal(str)                # media_id
    ...
    def __init__(self):
        ...
        self.list.itemDoubleClicked.connect(
            lambda _: self.addRequested.emit(self.selected_media_id() or ""))
        layout.addWidget(self.list, 1)

        btn_base = QPushButton("Import base…")
        btn_base.clicked.connect(lambda: self.importRequested.emit("main"))
        btn_motion = QPushButton("Import motion…")
        btn_motion.clicked.connect(lambda: self.importRequested.emit("motion"))
        row = QHBoxLayout()
        row.addWidget(btn_base)
        row.addWidget(btn_motion)
        layout.addLayout(row)

        add_btn = QPushButton("Add selected to timeline")
        add_btn.clicked.connect(
            lambda: self.addRequested.emit(self.selected_media_id() or ""))
        layout.addWidget(add_btn)

    def add_media(self, item) -> None:
        entry = QListWidgetItem(f"{item.label}  ·  {item.role}  ·  {item.nb_frames}f")
