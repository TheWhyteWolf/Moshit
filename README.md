...
Typical flow: **Import base** and **Import motion** → select each in the library
and **Add selected to timeline** → click the base clip → pick an effect (e.g.
`motion_splice`) and choose the motion source → **Apply & Preview** → scrub or
play the result → **Bake** to freeze it (reversible) → **Export…**.
...
through the preview (it also tracks playback). Two tools:
...
Delete (or right-click → Remove) takes a clip off the timeline. Timeline edits
apply on the next **Refresh preview**.

**Preview.** Frames are decoded with FFmpeg, so the GUI needs nothing beyond
PySide6 — no extra media libraries.
...
- Auto-refresh of the preview after timeline edits (currently manual).
- Frame-perfect on-demand preview via PyAV (v1 decodes to a cache).
